...
@export var smoke_projectile_scene: PackedScene
@export var rotation_speed: float = 12.0

@onready var shoot_point: Node2D = $ShootPoint
@onready var icon = load("res://assets/item_art/battants_smoke_grenade.png")
...
func _shoot() -> void:
	if has_been_used or owner_player == null:
		return
	if smoke_projectile_scene == null:
		push_warning("Assign a Flashbang projectile scene in the Inspector.")
		return

	var projectile := smoke_projectile_scene.instantiate()
